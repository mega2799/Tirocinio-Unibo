...
do 
	...
	do
		...
		echo -e "print(EJSON.stringify(db.getCollection('$col').explain('queryPlanner').aggregate([ { \$match: { \"$ind\" : $v}}]), null, \"  \")"  	>> "queries/on_"${col}"ind_"${ind}"_query_plan.js"
		
		echo -e "use('tirocinio')"												>> "queries/on_"${col}"ind_"${ind}"_exec_stats.js" 
		echo -e "print(EJSON.stringify(db.getCollection('$col').explain('executionStats').aggregate([ { \$match: { \"$ind\" : $v}}]), null, \" \"))" >> "queries/on_"${col}"ind_"${ind}"_exec_stats.js"	
