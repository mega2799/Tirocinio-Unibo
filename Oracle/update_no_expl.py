...
try:
    ...
    if connection:
        ...
        cursor.execute( "update " + collection + "set A7= 14 where AK = " + str(randint(0,9999)))
        ...
        for ind in ind_a:
            match ind:
                case 'A1':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set A7= 14 where " + ind + " = " + str(randint(0,9999)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'A2':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set A7= 14 where " + ind + " = " + str(randint(0,999)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'A3':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set A7= 14 where " + ind + " = " + str(randint(0,9)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'A4':
        ...
        # AK
        start_time = time.time()
        cursor.execute( "update " + collection + "set B7= 14 where BK = " + str(randint(0,9999)))
        t = (time.time() - start_time)
        t *= 1000
        ...
        for ind in ind_b:
            match ind:
                case 'B1':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set B7= 14 where " + ind + " = " + str(randint(0,9999)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'B2':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set B7= 14 where " + ind + " = " + str(randint(0,999)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'B3':
                    start_time = time.time()
                    cursor.execute( "update " + collection + "set B7= 14 where " + ind + " = " + str(randint(0,9)))
                    t = (time.time() - start_time)
                    t *= 1000
                    worksheet.write(get_colNum[ind] + 1, get_rowNum[collection] + 1 , int(t))
                case 'B4':
